...
exports.apiLogin = async function (req, res) {
  ...
  if (result) {
    let data = {
      token: jwt.sign({ _id: admin.data._id, name: admin.data.name, email: admin.data.email }, process.env.JWTSECRET, { expiresIn: tokenLasts }),
      id: admin.data._id,
...
exports.apiRegister = async function (req, res) {
  ...
  if (result) {
    let data = {
      token: jwt.sign({ _id: admin.data._id, name: admin.data.fName, email: admin.data.email }, process.env.JWTSECRET, { expiresIn: tokenLasts }),
      id: admin.data._id,
      name: admin.data.name,
      role: "admin"
    }
